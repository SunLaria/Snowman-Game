...
const buttonClickSound = new Audio("/static/pick-92276.mp3")
const winSound = new Audio("/static/success-fanfare-trumpets-6185.mp3")
const loseSound = new Audio("/static/fail-144746.mp3")
const gameStartSound= new Audio("/static/game-start-6104.mp3")
...
function setDef(word){
    ...
    .then(response => {
        document.getElementById("word-defenition").innerHTML=response.data[0].meanings[0].definitions.slice(0,1).map(item => item.definition).join(", ");
...
function update_snowman(fails){
    ...
    if(snowman.classList.contains("bounce")){
        snowman.classList.remove("bounce");
    }
    setTimeout(() => {
        snowman.classList.add("bounce");
    }, 1)
...
// end game function
function end_game(status,game_word){
    ...
    document.getElementById("hidden-word").innerHTML=game_word
...
function startGame(){
    ...
    document.getElementById("word-defenition").innerHTML=""
    ...
    for (let index = 0; index < letters.length; index++) {
        ...
        button.addEventListener("click",()=>{
            ...
            if (check_game_status(compare,fails)) {
                end_game(check_game_status(compare,fails),game_word);
            }
